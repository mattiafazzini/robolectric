...
public class ShadowTypedArray {
    ...
    public static TypedArray create(Resources realResources, int[] attrs, int[] data, int[] indices, int len, CharSequence[] stringData) {
        ...
        if (RuntimeEnvironment.getApiLevel() >= Build.VERSION_CODES.O) {
            typedArray = ReflectionHelpers.callConstructor(TypedArray.class, ClassParameter.from(Resources.class, realResources));
            ...
            typedArray = ReflectionHelpers.callConstructor(TypedArray.class, ClassParameter.from(Resources.class, realResources), ClassParameter.from(int[].class, data), ClassParameter.from(int[].class, indices), ClassParameter.from(int.class, len));
        }
    ...
    protected CharSequence loadStringValueAt(int index) {
        System.out.println("ShadowTypedArray#loadStringValueAt");
        if (ShadowAssetManager.useLegacy()) {
            return stringData[index / STYLE_NUM_ENTRIES];
        } else {
            return directlyOn(realTypedArray, TypedArray.class, "loadStringValueAt", new ClassParameter(int.class, index));
        }
    }

    @Implementation
    protected String getNonResourceString(@StyleableRes int index) {
        System.out.println("ShadowTypedArray#getNonResourceString");
    ...
    protected String getNonConfigurationString(@StyleableRes int index, int allowedChangingConfigs) {
        System.out.println("ShadowTypedArray#getNonConfigurationString");
        return directlyOn(realTypedArray, TypedArray.class).getString(index);
    }

    @Implementation
    protected String getPositionDescription() {
        System.out.println("ShadowTypedArray#getPositionDescription");
        if (ShadowAssetManager.useLegacy()) {
    ...
    private static final ImmutableMap<Integer, String> TYPE_MAP = ImmutableMap.<Integer, String>builder().put(TypedValue.TYPE_NULL, "TYPE_NULL").put(TypedValue.TYPE_REFERENCE, "TYPE_REFERENCE").put(TypedValue.TYPE_ATTRIBUTE, "TYPE_ATTRIBUTE").put(TypedValue.TYPE_STRING, "TYPE_STRING").put(TypedValue.TYPE_FLOAT, "TYPE_FLOAT").put(TypedValue.TYPE_DIMENSION, "TYPE_DIMENSION").put(TypedValue.TYPE_FRACTION, "TYPE_FRACTION").put(TypedValue.TYPE_INT_DEC, "TYPE_INT_DEC").put(TypedValue.TYPE_INT_HEX, "TYPE_INT_HEX").put(TypedValue.TYPE_INT_BOOLEAN, "TYPE_INT_BOOLEAN").put(TypedValue.TYPE_INT_COLOR_ARGB8, "TYPE_INT_COLOR_ARGB8").put(TypedValue.TYPE_INT_COLOR_RGB8, "TYPE_INT_COLOR_RGB8").put(TypedValue.TYPE_INT_COLOR_ARGB4, "TYPE_INT_COLOR_ARGB4").put(TypedValue.TYPE_INT_COLOR_RGB4, "TYPE_INT_COLOR_RGB4").build();
}
