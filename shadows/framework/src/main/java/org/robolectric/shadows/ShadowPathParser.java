...
public class ShadowPathParser {
    ...
    protected static Path createPathFromPathData(String pathData) {
        System.out.println("ShadowPathParser#createPathFromPathData");
        Path path = new Path();
    ...
    @Implementation
    protected static boolean interpolatePathData(PathData outData, PathData fromData, PathData toData, float fraction) {
        System.out.println("ShadowPathParser#interpolatePathData");
        return true;
    }

    @Implementation
    public static boolean nCanMorph(long fromDataPtr, long toDataPtr) {
        System.out.println("ShadowPathParser#nCanMorph");
        return true;
    }
    ...
    public static class PathDataNode {
        ...
        private static void addCommand(Path path, float[] current, char previousCmd, char cmd, float[] val) {
            ...
            for (int k = 0; k < val.length; k += incr) {
                switch(cmd) {
                    case // moveto - Start a new sub-path (relative)
                    'm':
                        path.rMoveTo(val[k + 0], val[k + 1]);
                        currentX += val[k + 0];
                        currentY += val[k + 1];
                        break;
                    case // moveto - Start a new sub-path
                    'M':
                        path.moveTo(val[k + 0], val[k + 1]);
                        currentX = val[k + 0];
                        currentY = val[k + 1];
                        break;
                    case // lineto - Draw a line from the current point (relative)
                    'l':
                        path.rLineTo(val[k + 0], val[k + 1]);
                        currentX += val[k + 0];
                        currentY += val[k + 1];
                        break;
                    case // lineto - Draw a line from the current point
                    'L':
                    ...
                    case 'z':
                    case // closepath - Close the current subpath
                    'Z':
                        path.close();
                        break;
                    case // horizontal lineto - Draws a horizontal line (relative)
                    'h':
                        path.rLineTo(val[k + 0], 0);
                        currentX += val[k + 0];
                        break;
                    case // horizontal lineto - Draws a horizontal line
                    'H':
                        path.lineTo(val[k + 0], currentY);
                        currentX = val[k + 0];
                        break;
                    case // vertical lineto - Draws a vertical line from the current point (r)
                    'v':
                        path.rLineTo(0, val[k + 0]);
                        currentY += val[k + 0];
                        break;
                    case // vertical lineto - Draws a vertical line from the current point
                    'V':
                        path.lineTo(currentX, val[k + 0]);
                        currentY = val[k + 0];
                        break;
                    case // curveto - Draws a cubic Bézier curve (relative)
                    'c':
                        ...
                        currentY += val[k + 5];
                        break;
                    case // curveto - Draws a cubic Bézier curve
                    'C':
                        ...
                        ctrlPointY = val[k + 3];
                        break;
                    case // smooth curveto - Draws a cubic Bézier curve (reflective cp)
                    's':
                        ...
                        currentY += val[k + 3];
                        break;
                    case // shorthand/smooth curveto Draws a cubic Bézier curve(reflective cp)
                    'S':
                        ...
                        currentY = val[k + 3];
                        break;
                    case // Draws a quadratic Bézier (relative)
                    'q':
                    ...
                    'Q':
                        ...
                        currentY = val[k + 3];
                        break;
                    case // Draws a quadratic Bézier curve(reflective control point)(relative)
                    't':
                        reflectiveCtrlPointX = 0;
                        reflectiveCtrlPointY = 0;
                        ...
                        currentY += val[k + 1];
                        break;
                    case // Draws a quadratic Bézier curve (reflective control point)
                    'T':
                        reflectiveCtrlPointX = currentX;
                        reflectiveCtrlPointY = currentY;
                        if (previousCmd == 'q' || previousCmd == 't' || previousCmd == 'Q' || previousCmd == 'T') {
    ...
    public static class ShadowPathData {
        ...
        public long getNativePtr() {
            System.out.println("ShadowPathData#getNativePtr");
        ...
        protected void finalize() throws Throwable {
            System.out.println("ShadowPathData#finalize");
