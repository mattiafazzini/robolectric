...
public class ShadowUserManager {
    ...
    protected void __constructor__(Context context, IUserManager service) {
        System.out.println("ShadowUserManager#__constructor__");
    ...
    protected Bundle getApplicationRestrictions(String packageName) {
        System.out.println("ShadowUserManager#getApplicationRestrictions");
    ...
    protected List<UserHandle> getUserProfiles() {
        System.out.println("ShadowUserManager#getUserProfiles");
    ...
    protected List<UserInfo> getProfiles(int userHandle) {
        System.out.println("ShadowUserManager#getProfiles");
        if (profiles.containsKey(userHandle)) {
            return ImmutableList.copyOf(profiles.get(userHandle));
        }
        return directlyOn(realObject, UserManager.class, "getProfiles", ClassParameter.from(int.class, userHandle));
    }
    ...
    protected boolean isUserUnlocked() {
        System.out.println("ShadowUserManager#isUserUnlocked");
        return userUnlocked;
    }
    ...
    protected boolean isManagedProfile() {
        System.out.println("ShadowUserManager#isManagedProfile");
    ...
    protected boolean hasUserRestriction(String restrictionKey, UserHandle userHandle) {
        System.out.println("ShadowUserManager#hasUserRestriction");
    ...
    protected Bundle getUserRestrictions(UserHandle userHandle) {
        System.out.println("ShadowUserManager#getUserRestrictions");
    ...
    protected long getSerialNumberForUser(UserHandle userHandle) {
        System.out.println("ShadowUserManager#getSerialNumberForUser");
    ...
    protected UserHandle getUserForSerialNumber(long serialNumber) {
        System.out.println("ShadowUserManager#getUserForSerialNumber");
    ...
    protected int getUserSerialNumber(@UserIdInt int userHandle) {
        System.out.println("ShadowUserManager#getUserSerialNumber");
    ...
    protected boolean isDemoUser() {
        System.out.println("ShadowUserManager#isDemoUser");
    ...
    protected boolean isSystemUser() {
        System.out.println("ShadowUserManager#isSystemUser");
    ...
    protected boolean isLinkedUser() {
        System.out.println("ShadowUserManager#isLinkedUser");
    ...
    protected boolean isUserRunning(UserHandle handle) {
        System.out.println("ShadowUserManager#isUserRunning");
        checkPermissions();
        UserState state = userState.get(handle.getIdentifier());
        if (state == UserState.STATE_RUNNING_LOCKED || state == UserState.STATE_RUNNING_UNLOCKED || state == UserState.STATE_RUNNING_UNLOCKING) {
            return true;
        } else {
            return false;
        }
    ...
    protected boolean isUserRunningOrStopping(UserHandle handle) {
        System.out.println("ShadowUserManager#isUserRunningOrStopping");
        checkPermissions();
        UserState state = userState.get(handle.getIdentifier());
        if (state == UserState.STATE_RUNNING_LOCKED || state == UserState.STATE_RUNNING_UNLOCKED || state == UserState.STATE_RUNNING_UNLOCKING || state == UserState.STATE_STOPPING) {
            return true;
        } else {
            return false;
        }
    ...
    protected List<UserInfo> getUsers() {
        System.out.println("ShadowUserManager#getUsers");
        return new ArrayList<UserInfo>(userInfoMap.values());
    }

    @Implementation
    protected UserInfo getUserInfo(int userHandle) {
        System.out.println("ShadowUserManager#getUserInfo");
    ...
    public void setCanSwitchUser(boolean canSwitchUser) {
        setUserSwitchability(canSwitchUser ? UserManager.SWITCHABILITY_STATUS_OK : UserManager.SWITCHABILITY_STATUS_USER_SWITCH_DISALLOWED);
    }

    @Implementation(minSdk = Build.VERSION_CODES.Q)
    protected String getSeedAccountType() {
        System.out.println("ShadowUserManager#getSeedAccountType");
    ...
    protected boolean removeUser(int userHandle) {
        System.out.println("ShadowUserManager#removeUser");
    ...
    protected static boolean supportsMultipleUsers() {
        System.out.println("ShadowUserManager#supportsMultipleUsers");
    ...
    public UserHandle addUser(int id, String name, int flags) {
        UserHandle userHandle = id == UserHandle.USER_SYSTEM ? Process.myUserHandle() : new UserHandle(id);
        addUserProfile(userHandle);
        setSerialNumberForUser(userHandle, (long) id);
        userInfoMap.put(id, new UserInfo(id, name, flags));
        userPidMap.put(id, id == UserHandle.USER_SYSTEM ? Process.myUid() : id * UserHandle.PER_USER_RANGE + ShadowProcess.getRandomApplicationUid());
        return userHandle;
    }
    ...
    protected boolean canSwitchUsers() {
        System.out.println("ShadowUserManager#canSwitchUsers");
        return getUserSwitchability() == UserManager.SWITCHABILITY_STATUS_OK;
    }

    @Implementation(minSdk = Q)
    protected int getUserSwitchability() {
        System.out.println("ShadowUserManager#getUserSwitchability");
