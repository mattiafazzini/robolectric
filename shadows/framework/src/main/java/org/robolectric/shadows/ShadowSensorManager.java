...
public class ShadowSensorManager {
    ...
    protected Sensor getDefaultSensor(int type) {
        System.out.println("ShadowSensorManager#getDefaultSensor");
        return sensorMap.get(type);
    }

    @Implementation
    public List<Sensor> getSensorList(int type) {
        System.out.println("ShadowSensorManager#getSensorList");
    ...
    protected boolean registerListener(SensorEventListener listener, Sensor sensor, int rate, Handler handler) {
        System.out.println("ShadowSensorManager#registerListener");
    ...
    protected void unregisterListener(SensorEventListener listener, Sensor sensor) {
        System.out.println("ShadowSensorManager#unregisterListener");
    ...
    protected Object createDirectChannel(MemoryFile mem) {
        System.out.println("ShadowSensorManager#createDirectChannel");
        return ReflectionHelpers.callConstructor(SensorDirectChannel.class, ClassParameter.from(SensorManager.class, realObject), ClassParameter.from(int.class, 0), ClassParameter.from(int.class, SensorDirectChannel.TYPE_MEMORY_FILE), ClassParameter.from(long.class, mem.length()));
    }
}
