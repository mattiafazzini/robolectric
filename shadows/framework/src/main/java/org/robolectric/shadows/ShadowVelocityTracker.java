...
public class ShadowVelocityTracker {
    ...
    protected void clear() {
        System.out.println("ShadowVelocityTracker#clear");
        maybeInitialize();
        curIndex = 0;
    ...
    protected void addMovement(MotionEvent event) {
        System.out.println("ShadowVelocityTracker#addMovement");
        maybeInitialize();
        if (event == null) {
    ...
    protected void computeCurrentVelocity(int units) {
        System.out.println("ShadowVelocityTracker#computeCurrentVelocity");
        ...
        for (int pointerId : newestMovement.pointerIds) {
            ...
            while (numTouches-- > 1) {
                ...
                if (duration >= MIN_DURATION) {
                    // one over time delta in seconds
                    float scale = 1000f / duration;
    ...
    protected float getXVelocity() {
        System.out.println("ShadowVelocityTracker#getXVelocity");
        return getXVelocity(ACTIVE_POINTER_ID);
    }

    @Implementation
    protected float getYVelocity() {
        System.out.println("ShadowVelocityTracker#getYVelocity");
