...
public class ShadowBitmap {
    ...
    protected boolean compress(Bitmap.CompressFormat format, int quality, OutputStream stream) {
        System.out.println("ShadowBitmap#compress");
    ...
    protected static Bitmap createBitmap(int width, int height, Bitmap.Config config) {
        System.out.println("ShadowBitmap#createBitmap");
    ...
    protected static Bitmap createScaledBitmap(Bitmap src, int dstWidth, int dstHeight, boolean filter) {
        System.out.println("ShadowBitmap#createScaledBitmap");
    ...
    protected void setPixels(int[] pixels, int offset, int stride, int x, int y, int width, int height) {
        System.out.println("ShadowBitmap#setPixels");
        this.colors = pixels;
    }
    ...
    protected int getPixel(int x, int y) {
        System.out.println("ShadowBitmap#getPixel");
    ...
    protected void setPixel(int x, int y, int color) {
        System.out.println("ShadowBitmap#setPixel");
        if (isRecycled()) {
    ...
    protected void getPixels(int[] pixels, int offset, int stride, int x, int y, int width, int height) {
        System.out.println("ShadowBitmap#getPixels");
        if (x != 0 || y != 0 || width != getWidth() || height != getHeight() || pixels.length != colors.length) {
    ...
    protected int getRowBytes() {
        System.out.println("ShadowBitmap#getRowBytes");
        return getBytesPerPixel(config) * getWidth();
    }

    @Implementation
    protected int getByteCount() {
        System.out.println("ShadowBitmap#getByteCount");
        return getRowBytes() * getHeight();
    }

    @Implementation
    protected void recycle() {
        System.out.println("ShadowBitmap#recycle");
        recycled = true;
    }

    @Implementation
    protected final boolean isRecycled() {
        System.out.println("ShadowBitmap#isRecycled");
        return recycled;
    }

    @Implementation
    protected Bitmap copy(Bitmap.Config config, boolean isMutable) {
        System.out.println("ShadowBitmap#copy");
        Bitmap newBitmap = ReflectionHelpers.callConstructor(Bitmap.class);
        ShadowBitmap shadowBitmap = Shadow.extract(newBitmap);
    ...
    protected final int getAllocationByteCount() {
        System.out.println("ShadowBitmap#getAllocationByteCount");
        return getRowBytes() * getHeight();
    }

    @Implementation
    protected final Bitmap.Config getConfig() {
        System.out.println("ShadowBitmap#getConfig");
        return config;
    }

    @Implementation(minSdk = KITKAT)
    protected void setConfig(Bitmap.Config config) {
        System.out.println("ShadowBitmap#setConfig");
        this.config = config;
    }

    @Implementation
    protected final boolean isMutable() {
        System.out.println("ShadowBitmap#isMutable");
        return mutable;
    }
    ...
    protected final boolean hasAlpha() {
        System.out.println("ShadowBitmap#hasAlpha");
        return hasAlpha;
    }

    @Implementation
    protected void setHasAlpha(boolean hasAlpha) {
        System.out.println("ShadowBitmap#setHasAlpha");
        this.hasAlpha = hasAlpha;
    }

    @Implementation
    protected Bitmap extractAlpha() {
        System.out.println("ShadowBitmap#extractAlpha");
    ...
    protected final boolean hasMipMap() {
        System.out.println("ShadowBitmap#hasMipMap");
        return hasMipMap;
    }

    @Implementation(minSdk = JELLY_BEAN_MR1)
    protected final void setHasMipMap(boolean hasMipMap) {
        System.out.println("ShadowBitmap#setHasMipMap");
        this.hasMipMap = hasMipMap;
    }

    @Implementation(minSdk = KITKAT)
    protected void setWidth(int width) {
        System.out.println("ShadowBitmap#setWidth");
        this.width = width;
    }

    @Implementation
    protected int getWidth() {
        System.out.println("ShadowBitmap#getWidth");
        return width;
    }

    @Implementation(minSdk = KITKAT)
    protected void setHeight(int height) {
        System.out.println("ShadowBitmap#setHeight");
        this.height = height;
    }

    @Implementation
    protected int getHeight() {
        System.out.println("ShadowBitmap#getHeight");
        return height;
    }

    @Implementation
    protected void setDensity(int density) {
        System.out.println("ShadowBitmap#setDensity");
        this.density = density;
    }

    @Implementation
    protected int getDensity() {
        System.out.println("ShadowBitmap#getDensity");
        return density;
    }

    @Implementation
    protected int getGenerationId() {
        System.out.println("ShadowBitmap#getGenerationId");
        return 0;
    }

    @Implementation(minSdk = M)
    protected Bitmap createAshmemBitmap() {
        System.out.println("ShadowBitmap#createAshmemBitmap");
        return realBitmap;
    }

    @Implementation
    protected void eraseColor(int color) {
        System.out.println("ShadowBitmap#eraseColor");
        if (colors != null) {
    ...
    protected void writeToParcel(Parcel p, int flags) {
        System.out.println("ShadowBitmap#writeToParcel");
        p.writeInt(width);
        p.writeInt(height);
    ...
    protected static Bitmap nativeCreateFromParcel(Parcel p) {
        System.out.println("ShadowBitmap#nativeCreateFromParcel");
    ...
    protected void copyPixelsFromBuffer(Buffer dst) {
        System.out.println("ShadowBitmap#copyPixelsFromBuffer");
        if (isRecycled()) {
        ...
        if (getBytesPerPixel(config) != INTERNAL_BYTES_PER_PIXEL) {
            throw new RuntimeException("Not implemented: only Bitmaps with " + INTERNAL_BYTES_PER_PIXEL + " bytes per pixel are supported");
        }
    ...
    protected void copyPixelsToBuffer(Buffer dst) {
        System.out.println("ShadowBitmap#copyPixelsToBuffer");
    ...
    protected void reconfigure(int width, int height, Bitmap.Config config) {
        System.out.println("ShadowBitmap#reconfigure");
    ...
    protected void setPremultiplied(boolean isPremultiplied) {
        System.out.println("ShadowBitmap#setPremultiplied");
        this.isPremultiplied = isPremultiplied;
    }

    @Implementation(minSdk = KITKAT)
    protected boolean isPremultiplied() {
        System.out.println("ShadowBitmap#isPremultiplied");
        return isPremultiplied;
    }

    @Implementation
    protected boolean sameAs(Bitmap other) {
        System.out.println("ShadowBitmap#sameAs");
        if (other == null) {
            return false;
        }
        ...
        if (this.config != null && shadowOtherBitmap.config != null && this.config != shadowOtherBitmap.config) {
            return false;
        }
