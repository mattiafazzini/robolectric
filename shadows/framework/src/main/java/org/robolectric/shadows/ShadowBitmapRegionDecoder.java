...
public class ShadowBitmapRegionDecoder {
    ...
    protected static BitmapRegionDecoder newInstance(byte[] data, int offset, int length, boolean isShareable) throws IOException {
        System.out.println("ShadowBitmapRegionDecoder#newInstance");
    ...
    protected int getWidth() {
        System.out.println("ShadowBitmapRegionDecoder#getWidth");
        return width;
    }

    @Implementation
    protected int getHeight() {
        System.out.println("ShadowBitmapRegionDecoder#getHeight");
        return height;
    }

    @Implementation
    protected Bitmap decodeRegion(Rect rect, BitmapFactory.Options options) {
        System.out.println("ShadowBitmapRegionDecoder#decodeRegion");
    ...
    private static BitmapRegionDecoder newInstance() {
        if (getApiLevel() >= LOLLIPOP) {
            return ReflectionHelpers.callConstructor(BitmapRegionDecoder.class, new ReflectionHelpers.ClassParameter<>(long.class, 0L));
        } else {
            return ReflectionHelpers.callConstructor(BitmapRegionDecoder.class, new ReflectionHelpers.ClassParameter<>(int.class, 0));
        }
    }
}
