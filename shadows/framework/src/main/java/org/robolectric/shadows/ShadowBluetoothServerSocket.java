...
public class ShadowBluetoothServerSocket {
    ...
    public static BluetoothServerSocket newInstance(int type, boolean auth, boolean encrypt, ParcelUuid uuid) {
        if (Build.VERSION.SDK_INT >= JELLY_BEAN_MR1) {
            return Shadow.newInstance(BluetoothServerSocket.class, new Class<?>[] { Integer.TYPE, Boolean.TYPE, Boolean.TYPE, ParcelUuid.class }, new Object[] { type, auth, encrypt, uuid });
        } else {
            return Shadow.newInstance(BluetoothServerSocket.class, new Class<?>[] { Integer.TYPE, Boolean.TYPE, Boolean.TYPE, Integer.TYPE }, new Object[] { type, auth, encrypt, getPort(uuid) });
        }
    }
    ...
    protected BluetoothSocket accept(int timeout) throws IOException {
        System.out.println("ShadowBluetoothServerSocket#accept");
        if (closed) {
    ...
    protected void close() throws IOException {
        System.out.println("ShadowBluetoothServerSocket#close");
        closed = true;
    }
