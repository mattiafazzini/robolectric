...
public class ShadowContentResolver {
    ...
    private static class ContentObserverEntry {
        ...
        public boolean matches(Uri test) {
            ...
            return Objects.equals(uriPath, testPath) || (notifyForDescendents && testPath != null && testPath.startsWith(uriPath));
        }
    }
    ...
    protected final InputStream openInputStream(final Uri uri) {
        System.out.println("ShadowContentResolver#openInputStream");
    ...
    protected final OutputStream openOutputStream(final Uri uri) {
        System.out.println("ShadowContentResolver#openOutputStream");
    ...
    protected final Uri insert(Uri url, ContentValues values) {
        System.out.println("ShadowContentResolver#insert");
    ...
    protected int update(Uri uri, ContentValues values, String where, String[] selectionArgs) {
        System.out.println("ShadowContentResolver#update");
    ...
    protected final Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder) {
        System.out.println("ShadowContentResolver#query");
        ContentProvider provider = getProvider(uri);
        if (provider != null) {
    ...
    protected String getType(Uri uri) {
        System.out.println("ShadowContentResolver#getType");
        ContentProvider provider = getProvider(uri);
        if (provider != null) {
    ...
    protected Bundle call(Uri uri, String method, String arg, Bundle extras) {
        System.out.println("ShadowContentResolver#call");
    ...
    protected final ContentProviderClient acquireContentProviderClient(String name) {
        System.out.println("ShadowContentResolver#acquireContentProviderClient");
    ...
    protected final ContentProviderClient acquireUnstableContentProviderClient(String name) {
        System.out.println("ShadowContentResolver#acquireUnstableContentProviderClient");
        ContentProvider provider = getProvider(name);
        if (provider == null) {
            return null;
        }
    ...
    protected final IContentProvider acquireProvider(String name) {
        System.out.println("ShadowContentResolver#acquireProvider");
    ...
    protected final IContentProvider acquireUnstableProvider(String name) {
        System.out.println("ShadowContentResolver#acquireUnstableProvider");
    ...
    protected final int delete(Uri url, String where, String[] selectionArgs) {
        System.out.println("ShadowContentResolver#delete");
        ContentProvider provider = getProvider(url);
    ...
    protected final int bulkInsert(Uri url, ContentValues[] values) {
        System.out.println("ShadowContentResolver#bulkInsert");
        ContentProvider provider = getProvider(url);
    ...
    protected void notifyChange(Uri uri, ContentObserver observer, boolean syncToNetwork) {
        System.out.println("ShadowContentResolver#notifyChange");
    ...
    protected ContentProviderResult[] applyBatch(String authority, ArrayList<ContentProviderOperation> operations) throws OperationApplicationException {
        System.out.println("ShadowContentResolver#applyBatch");
    ...
    protected static void requestSync(Account account, String authority, Bundle extras) {
        System.out.println("ShadowContentResolver#requestSync");
    ...
    protected static void cancelSync(Account account, String authority) {
        System.out.println("ShadowContentResolver#cancelSync");
    ...
    protected static boolean isSyncActive(Account account, String authority) {
        System.out.println("ShadowContentResolver#isSyncActive");
    ...
    protected static boolean getSyncAutomatically(Account account, String authority) {
        System.out.println("ShadowContentResolver#getSyncAutomatically");
    ...
    protected static void setSyncAutomatically(Account account, String authority, boolean sync) {
        System.out.println("ShadowContentResolver#setSyncAutomatically");
    ...
    protected static void addPeriodicSync(Account account, String authority, Bundle extras, long pollFrequency) {
        System.out.println("ShadowContentResolver#addPeriodicSync");
        validateSyncExtrasBundle(extras);
    ...
    protected static void removePeriodicSync(Account account, String authority, Bundle extras) {
        System.out.println("ShadowContentResolver#removePeriodicSync");
        validateSyncExtrasBundle(extras);
        Status status = getStatus(account, authority);
        if (status != null) {
    ...
    protected static List<PeriodicSync> getPeriodicSyncs(Account account, String authority) {
        System.out.println("ShadowContentResolver#getPeriodicSyncs");
        return getStatus(account, authority, true).syncs;
    }

    @Implementation
    protected static void validateSyncExtrasBundle(Bundle extras) {
        System.out.println("ShadowContentResolver#validateSyncExtrasBundle");
    ...
    protected static void setMasterSyncAutomatically(boolean sync) {
        System.out.println("ShadowContentResolver#setMasterSyncAutomatically");
        masterSyncAutomatically = sync;
    }

    @Implementation
    protected static boolean getMasterSyncAutomatically() {
        System.out.println("ShadowContentResolver#getMasterSyncAutomatically");
        return masterSyncAutomatically;
    }

    @Implementation(minSdk = KITKAT)
    protected void takePersistableUriPermission(@NonNull Uri uri, int modeFlags) {
        System.out.println("ShadowContentResolver#takePersistableUriPermission");
    ...
    protected void releasePersistableUriPermission(@NonNull Uri uri, int modeFlags) {
        System.out.println("ShadowContentResolver#releasePersistableUriPermission");
        Objects.requireNonNull(uri, "uri may not be null");
        modeFlags &= (Intent.FLAG_GRANT_READ_URI_PERMISSION | Intent.FLAG_GRANT_WRITE_URI_PERMISSION);
        // If neither read nor write permission is specified there is nothing to do.
        if (modeFlags == 0) {
        ...
        for (Iterator<UriPermission> i = uriPermissions.iterator(); i.hasNext(); ) {
            ...
            if (uri.equals(perm.getUri())) {
                ...
                int oldModeFlags = (perm.isReadPermission() ? Intent.FLAG_GRANT_READ_URI_PERMISSION : 0) | (perm.isWritePermission() ? Intent.FLAG_GRANT_WRITE_URI_PERMISSION : 0);
    ...
    protected List<UriPermission> getPersistedUriPermissions() {
        System.out.println("ShadowContentResolver#getPersistedUriPermissions");
        return uriPermissions;
    }

    private void addUriPermission(@NonNull Uri uri, int modeFlags) {
        UriPermission perm = ReflectionHelpers.callConstructor(UriPermission.class, ClassParameter.from(Uri.class, uri), ClassParameter.from(int.class, modeFlags), ClassParameter.from(long.class, System.currentTimeMillis()));
    ...
    private static synchronized ContentProvider getProvider(String authority) {
        if (!providers.containsKey(authority)) {
            ProviderInfo providerInfo = RuntimeEnvironment.application.getPackageManager().resolveContentProvider(authority, 0);
    ...
    protected void registerContentObserver(Uri uri, boolean notifyForDescendents, ContentObserver observer) {
        System.out.println("ShadowContentResolver#registerContentObserver");
        if (uri == null || observer == null) {
            throw new NullPointerException();
        }
    ...
    protected void unregisterContentObserver(ContentObserver observer) {
        System.out.println("ShadowContentResolver#unregisterContentObserver");
    ...
    protected static SyncAdapterType[] getSyncAdapterTypes() {
        System.out.println("ShadowContentResolver#getSyncAdapterTypes");
