...
public class ShadowFilter {
    ...
    protected void filter(CharSequence constraint, Filter.FilterListener listener) {
        System.out.println("ShadowFilter#filter");
        try {
            Class<?> forName = Class.forName("android.widget.Filter$FilterResults");
            Object filtering;
            try {
                filtering = ReflectionHelpers.callInstanceMethod(realObject, "performFiltering", ClassParameter.from(CharSequence.class, constraint));
            } catch (Exception e) {
                e.printStackTrace();
