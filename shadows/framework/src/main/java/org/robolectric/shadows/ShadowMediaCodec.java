...
public class ShadowMediaCodec {
    ...
    private final ByteBuffer[] inputBuffers = new ByteBuffer[] { ByteBuffer.wrap(new byte[BUFFER_SIZE]) };

    private final ByteBuffer[] outputBuffers = new ByteBuffer[] { ByteBuffer.wrap(new byte[BUFFER_SIZE]) };
    ...
    protected void native_setCallback(MediaCodec.Callback callback) {
        System.out.println("ShadowMediaCodec#native_setCallback");
    ...
    protected void native_start() {
        System.out.println("ShadowMediaCodec#native_start");
    ...
    protected ByteBuffer[] getBuffers(boolean input) {
        System.out.println("ShadowMediaCodec#getBuffers");
    ...
    protected ByteBuffer getBuffer(boolean input, int index) {
        System.out.println("ShadowMediaCodec#getBuffer");
    ...
    protected void native_queueInputBuffer(int index, int offset, int size, long presentationTimeUs, int flags) {
        System.out.println("ShadowMediaCodec#native_queueInputBuffer");
    ...
    protected void releaseOutputBuffer(int index, boolean render, boolean updatePTS, long timeNs) {
        System.out.println("ShadowMediaCodec#releaseOutputBuffer");
        if (reachedEos) {
            return;
        }
    ...
    private void postFakeNativeEvent(int what, int arg1, int arg2, @Nullable Object obj) {
        ReflectionHelpers.callInstanceMethod(MediaCodec.class, realCodec, "postEventFromNative", ReflectionHelpers.ClassParameter.from(int.class, what), ReflectionHelpers.ClassParameter.from(int.class, arg1), ReflectionHelpers.ClassParameter.from(int.class, arg2), ReflectionHelpers.ClassParameter.from(Object.class, obj));
    }
     ...
     */
    @Implementation(minSdk = LOLLIPOP)
    protected void validateOutputByteBuffer(@Nullable ByteBuffer[] buffers, int index, @NonNull BufferInfo info) {
        System.out.println("ShadowMediaCodec#validateOutputByteBuffer");
