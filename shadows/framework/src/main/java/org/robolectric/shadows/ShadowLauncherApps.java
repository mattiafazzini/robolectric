...
public class ShadowLauncherApps {
    ...
    private void shortcutsChanged(String packageName, List<ShortcutInfo> shortcuts) {
        for (Pair<LauncherApps.Callback, Handler> callbackPair : callbacks) {
            callbackPair.second.post(() -> callbackPair.first.onShortcutsChanged(packageName, shortcuts, Process.myUserHandle()));
        }
    }
    ...
    public void notifyPackageAdded(String packageName) {
        for (Pair<LauncherApps.Callback, Handler> callbackPair : callbacks) {
            callbackPair.second.post(() -> callbackPair.first.onPackageAdded(packageName, Process.myUserHandle()));
        }
    }
    ...
    public void notifyPackageRemoved(String packageName) {
        for (Pair<LauncherApps.Callback, Handler> callbackPair : callbacks) {
            callbackPair.second.post(() -> callbackPair.first.onPackageRemoved(packageName, Process.myUserHandle()));
        }
    }

    @Implementation(minSdk = Q)
    protected void startPackageInstallerSessionDetailsActivity(@NonNull SessionInfo sessionInfo, @Nullable Rect sourceBounds, @Nullable Bundle opts) {
        System.out.println("ShadowLauncherApps#startPackageInstallerSessionDetailsActivity");
    ...
    protected void startAppDetailsActivity(ComponentName component, UserHandle user, Rect sourceBounds, Bundle opts) {
        System.out.println("ShadowLauncherApps#startAppDetailsActivity");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }
    ...
    protected List<LauncherActivityInfo> getShortcutConfigActivityList(@Nullable String packageName, @NonNull UserHandle user) {
        System.out.println("ShadowLauncherApps#getShortcutConfigActivityList");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }
    ...
    protected IntentSender getShortcutConfigActivityIntent(@NonNull LauncherActivityInfo info) {
        System.out.println("ShadowLauncherApps#getShortcutConfigActivityIntent");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }

    @Implementation
    protected boolean isPackageEnabled(String packageName, UserHandle user) {
        System.out.println("ShadowLauncherApps#isPackageEnabled");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }
    ...
    protected Bundle getSuspendedPackageLauncherExtras(String packageName, UserHandle user) {
        System.out.println("ShadowLauncherApps#getSuspendedPackageLauncherExtras");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }

    @Implementation(minSdk = Q)
    protected boolean shouldHideFromSuggestions(@NonNull String packageName, @NonNull UserHandle user) {
        System.out.println("ShadowLauncherApps#shouldHideFromSuggestions");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }

    @Implementation
    protected boolean isActivityEnabled(ComponentName component, UserHandle user) {
        System.out.println("ShadowLauncherApps#isActivityEnabled");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }

    @Implementation(minSdk = N)
    protected boolean hasShortcutHostPermission() {
        System.out.println("ShadowLauncherApps#hasShortcutHostPermission");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }
    ...
    protected List<ShortcutInfo> getShortcuts(@NonNull ShortcutQuery query, @NonNull UserHandle user) {
        System.out.println("ShadowLauncherApps#getShortcuts");
    ...
    protected void pinShortcuts(@NonNull String packageName, @NonNull List<String> shortcutIds, @NonNull UserHandle user) {
        System.out.println("ShadowLauncherApps#pinShortcuts");
        Iterable<ShortcutInfo> changed = Iterables.filter(shortcuts, shortcut -> !shortcutIds.contains(shortcut.getId()));
        List<ShortcutInfo> ret = Lists.newArrayList(changed);
        shortcuts = Lists.newArrayList(Iterables.filter(shortcuts, shortcut -> shortcutIds.contains(shortcut.getId())));
    ...
    protected void startShortcut(@NonNull String packageName, @NonNull String shortcutId, @Nullable Rect sourceBounds, @Nullable Bundle startActivityOptions, @NonNull UserHandle user) {
        System.out.println("ShadowLauncherApps#startShortcut");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }
    ...
    protected void registerCallback(LauncherApps.Callback callback) {
        System.out.println("ShadowLauncherApps#registerCallback");
    ...
    protected void registerCallback(LauncherApps.Callback callback, Handler handler) {
        System.out.println("ShadowLauncherApps#registerCallback");
        callbacks.add(Pair.create(callback, handler != null ? handler : new Handler(Looper.myLooper())));
    }

    @Implementation
    protected void unregisterCallback(LauncherApps.Callback callback) {
        System.out.println("ShadowLauncherApps#unregisterCallback");
    ...
    @Implementation(minSdk = Q)
    protected void registerPackageInstallerSessionCallback(@NonNull Executor executor, @NonNull SessionCallback callback) {
        System.out.println("ShadowLauncherApps#registerPackageInstallerSessionCallback");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }

    @Implementation(minSdk = Q)
    protected void unregisterPackageInstallerSessionCallback(@NonNull SessionCallback callback) {
        System.out.println("ShadowLauncherApps#unregisterPackageInstallerSessionCallback");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }
    ...
    protected List<SessionInfo> getAllPackageInstallerSessions() {
        System.out.println("ShadowLauncherApps#getAllPackageInstallerSessions");
        throw new UnsupportedOperationException("This method is not currently supported in Robolectric.");
    }
