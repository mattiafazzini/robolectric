...
public class ShadowDisplayEventReceiver {
    ...
    protected static long nativeInit(WeakReference<DisplayEventReceiver> receiver, MessageQueue msgQueue, int vsyncSource) {
        System.out.println("ShadowDisplayEventReceiver#nativeInit");
    ...
    protected static void nativeDispose(long receiverPtr) {
        System.out.println("ShadowDisplayEventReceiver#nativeDispose");
    ...
    protected static void nativeScheduleVsync(long receiverPtr) {
        System.out.println("ShadowDisplayEventReceiver#nativeScheduleVsync");
    ...
    protected void onVsync() {
        if (RuntimeEnvironment.getApiLevel() <= JELLY_BEAN) {
            ReflectionHelpers.callInstanceMethod(DisplayEventReceiver.class, receiver, "onVsync", ClassParameter.from(long.class, ShadowSystem.nanoTime()), ClassParameter.from(int.class, 1));
        } else if (RuntimeEnvironment.getApiLevel() < Q) {
            ReflectionHelpers.callInstanceMethod(DisplayEventReceiver.class, receiver, "onVsync", ClassParameter.from(long.class, ShadowSystem.nanoTime()), ClassParameter.from(int.class, 0), /* SurfaceControl.BUILT_IN_DISPLAY_ID_MAIN */
            ClassParameter.from(int.class, 1));
        } else {
            receiver.onVsync(ShadowSystem.nanoTime(), 0L, /* physicalDisplayId currently ignored */
            1);
        }
    }
