 ...
 */
@Implements(value = SystemClock.class, shadowPicker = ShadowSystemClock.Picker.class, // turn off shadowOf generation
...
public class ShadowLegacySystemClock extends ShadowSystemClock {
    ...
    protected static void sleep(long millis) {
        System.out.println("ShadowLegacySystemClock#sleep");
        if (ShadowApplication.getInstance() == null) {
            return;
        }
    ...
    protected static boolean setCurrentTimeMillis(long millis) {
        System.out.println("ShadowLegacySystemClock#setCurrentTimeMillis");
        if (ShadowApplication.getInstance() == null) {
            return false;
        }
        if (now() > millis) {
    ...
    protected static long uptimeMillis() {
        System.out.println("ShadowLegacySystemClock#uptimeMillis");
        return now() - bootedAt;
    }

    @Implementation
    protected static long elapsedRealtime() {
        System.out.println("ShadowLegacySystemClock#elapsedRealtime");
        return uptimeMillis();
    }

    @Implementation(minSdk = JELLY_BEAN_MR1)
    protected static long elapsedRealtimeNanos() {
        System.out.println("ShadowLegacySystemClock#elapsedRealtimeNanos");
        return elapsedRealtime() * MILLIS_PER_NANO;
    }

    @Implementation
    protected static long currentThreadTimeMillis() {
        System.out.println("ShadowLegacySystemClock#currentThreadTimeMillis");
        return uptimeMillis();
    }
    ...
    public static long currentThreadTimeMicro() {
        System.out.println("ShadowLegacySystemClock#currentThreadTimeMicro");
    ...
    public static long currentTimeMicro() {
        System.out.println("ShadowLegacySystemClock#currentTimeMicro");
        return now() * 1000;
    }
    ...
    protected static long currentNetworkTimeMillis() {
        System.out.println("ShadowLegacySystemClock#currentNetworkTimeMillis");
