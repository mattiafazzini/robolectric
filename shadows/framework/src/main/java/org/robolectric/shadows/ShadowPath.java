...
public class ShadowPath {
    ...
    protected void __constructor__(Path path) {
        System.out.println("ShadowPath#__constructor__");
    ...
    protected void moveTo(float x, float y) {
        System.out.println("ShadowPath#moveTo");
    ...
    protected void lineTo(float x, float y) {
        System.out.println("ShadowPath#lineTo");
        if (!hasPoints()) {
    ...
    protected void quadTo(float x1, float y1, float x2, float y2) {
        System.out.println("ShadowPath#quadTo");
        isSimplePath = false;
        if (!hasPoints()) {
            moveTo(0, 0);
        }
    ...
    protected void cubicTo(float x1, float y1, float x2, float y2, float x3, float y3) {
        System.out.println("ShadowPath#cubicTo");
        if (!hasPoints()) {
            mPath.moveTo(0, 0);
        }
    ...
    protected void reset() {
        System.out.println("ShadowPath#reset");
        mPath.reset();
        mLastX = 0;
        mLastY = 0;
        // Legacy recording behavior
    ...
    protected float[] approximate(float acceptableError) {
        System.out.println("ShadowPath#approximate");
    ...
    protected void rewind() {
        System.out.println("ShadowPath#rewind");
    ...
    protected boolean isConvex() {
        System.out.println("ShadowPath#isConvex");
    ...
    protected Path.FillType getFillType() {
        System.out.println("ShadowPath#getFillType");
        return mFillType;
    }

    @Implementation
    protected void setFillType(Path.FillType fillType) {
        System.out.println("ShadowPath#setFillType");
        mFillType = fillType;
    ...
    protected boolean isRect(RectF rect) {
        System.out.println("ShadowPath#isRect");
    ...
    protected void rLineTo(float dx, float dy) {
        System.out.println("ShadowPath#rLineTo");
        if (!hasPoints()) {
            mPath.moveTo(mLastX = 0, mLastY = 0);
        }
    ...
    protected void rQuadTo(float dx1, float dy1, float dx2, float dy2) {
        System.out.println("ShadowPath#rQuadTo");
        if (!hasPoints()) {
            mPath.moveTo(mLastX = 0, mLastY = 0);
        }
        dx1 += mLastX;
    ...
    protected void rCubicTo(float x1, float y1, float x2, float y2, float x3, float y3) {
        System.out.println("ShadowPath#rCubicTo");
        if (!hasPoints()) {
            mPath.moveTo(mLastX = 0, mLastY = 0);
        }
        x1 += mLastX;
    ...
    protected void arcTo(RectF oval, float startAngle, float sweepAngle) {
        System.out.println("ShadowPath#arcTo");
    ...
    protected void close() {
        System.out.println("ShadowPath#close");
        if (!hasPoints()) {
            mPath.moveTo(mLastX = 0, mLastY = 0);
        }
        mPath.closePath();
    ...
    protected void addRect(RectF rect, Direction dir) {
        System.out.println("ShadowPath#addRect");
    ...
    protected void addOval(float left, float top, float right, float bottom, Path.Direction dir) {
        System.out.println("ShadowPath#addOval");
    ...
    protected void addCircle(float x, float y, float radius, Path.Direction dir) {
        System.out.println("ShadowPath#addCircle");
    ...
    protected void addArc(float left, float top, float right, float bottom, float startAngle, float sweepAngle) {
        System.out.println("ShadowPath#addArc");
        mPath.append(new Arc2D.Float(left, top, right - left, bottom - top, -startAngle, -sweepAngle, Arc2D.OPEN), false);
    }

    @Implementation(minSdk = JELLY_BEAN)
    protected void addRoundRect(RectF rect, float rx, float ry, Direction dir) {
        System.out.println("ShadowPath#addRoundRect");
    ...
    protected void addRoundRect(float left, float top, float right, float bottom, float rx, float ry, Path.Direction dir) {
        System.out.println("ShadowPath#addRoundRect");
        mPath.append(new RoundRectangle2D.Float(left, top, right - left, bottom - top, rx * 2, ry * 2), false);
    }
    ...
    protected void addRoundRect(float left, float top, float right, float bottom, float[] radii, Path.Direction dir) {
        ...
        mPath.append(new RoundRectangle(left, top, right - left, bottom - top, cornerDimensions), false);
    }

    @Implementation
    protected void addPath(Path src, float dx, float dy) {
        System.out.println("ShadowPath#addPath");
        isSimplePath = false;
    ...
    protected void offset(float dx, float dy, Path dst) {
        System.out.println("ShadowPath#offset");
        if (dst != null) {
            dst.set(realObject);
        } else {
            dst = realObject;
    ...
    protected void setLastPoint(float dx, float dy) {
        System.out.println("ShadowPath#setLastPoint");
        mLastX = dx;
        mLastY = dy;
    }
    ...
    protected void transform(Matrix matrix, Path dst) {
        System.out.println("ShadowPath#transform");
        ShadowMatrix shadowMatrix = extract(matrix);
