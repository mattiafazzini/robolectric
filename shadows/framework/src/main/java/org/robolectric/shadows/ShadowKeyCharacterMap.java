...
public class ShadowKeyCharacterMap {
    ...
    protected int getKeyboardType() {
        System.out.println("ShadowKeyCharacterMap#getKeyboardType");
        return KeyCharacterMap.FULL;
    }

    @Implementation
    protected int get(int keyCode, int metaState) {
        System.out.println("ShadowKeyCharacterMap#get");
        boolean metaShiftOn = (metaState & KeyEvent.META_SHIFT_ON) != 0;
        Character character = KEY_CODE_TO_CHAR.get(keyCode);
        if (character == null) {
            return 0;
        } else if (metaShiftOn) {
            return KEY_CODE_TO_CHAR_SHIFT_ON.containsKey(keyCode) ? KEY_CODE_TO_CHAR_SHIFT_ON.get(keyCode) : character;
        } else {
    ...
    public KeyEvent getDownEvent(char a) {
        return new KeyEvent(0, 0, KeyEvent.ACTION_DOWN, toCharKeyCode(a), 0, getMetaState(a), KeyCharacterMap.VIRTUAL_KEYBOARD, 0);
    }

    public KeyEvent getUpEvent(char a) {
        return new KeyEvent(0, 0, KeyEvent.ACTION_UP, toCharKeyCode(a), 0, getMetaState(a), KeyCharacterMap.VIRTUAL_KEYBOARD, 0);
    }

    @Implementation(minSdk = KITKAT_WATCH)
    protected static char nativeGetNumber(long ptr, int keyCode) {
        System.out.println("ShadowKeyCharacterMap#nativeGetNumber");
        Character character = KEY_CODE_TO_CHAR.get(keyCode);
        if (character == null) {
            return 0;
        }
