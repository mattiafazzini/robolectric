 ...
 */
@Implements(value = Looper.class, // turn off shadowOf generation.
...
public final class ShadowPausedLooper extends ShadowLooper {
    ...
    private static Set<Looper> loopingLoopers = Collections.synchronizedSet(Collections.newSetFromMap(new WeakHashMap<Looper, Boolean>()));
    ...
    protected void __constructor__(boolean quitAllowed) {
        System.out.println("ShadowPausedLooper#__constructor__");
    ...
    public void runToEndOfTasks() {
        idleFor(getLastScheduledTaskTime().toMillis() - SystemClock.uptimeMillis(), TimeUnit.MILLISECONDS);
    }

    @Override
    public void runToNextTask() {
        idleFor(getNextScheduledTaskTime().toMillis() - SystemClock.uptimeMillis(), TimeUnit.MILLISECONDS);
    }
    ...
    protected static void prepareMainLooper() {
        System.out.println("ShadowPausedLooper#prepareMainLooper");
    ...
    @Implementation
    protected void quit() {
        System.out.println("ShadowPausedLooper#quit");
        if (isPaused()) {
            executeOnLooper(new UnPauseRunnable());
        }
    ...
    protected void quitSafely() {
        System.out.println("ShadowPausedLooper#quitSafely");
        if (isPaused()) {
            executeOnLooper(new UnPauseRunnable());
        }
