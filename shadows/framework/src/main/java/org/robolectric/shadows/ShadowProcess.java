...
public class ShadowProcess {
    ...
    protected static final int myPid() {
        System.out.println("ShadowProcess#myPid");
        return pid;
    }
    ...
    protected static final int myUid() {
        System.out.println("ShadowProcess#myUid");
        return uid;
    }
    ...
    protected static final int myTid() {
        System.out.println("ShadowProcess#myTid");
    ...
    protected static final void setThreadPriority(int priority) {
        System.out.println("ShadowProcess#setThreadPriority");
    ...
    protected static final void setThreadPriority(int tid, int priority) {
        System.out.println("ShadowProcess#setThreadPriority");
        checkArgument(priority >= android.os.Process.THREAD_PRIORITY_URGENT_AUDIO && priority <= android.os.Process.THREAD_PRIORITY_LOWEST, "priority %s out of range. Use a Process.THREAD_PRIORITY_* constant.", priority);
        if (tid == 0) {
    ...
    protected static final int getThreadPriority(int tid) {
        System.out.println("ShadowProcess#getThreadPriority");
        if (tid == 0) {
            tid = ShadowProcess.myTid();
        }
        synchronized (threadPrioritiesLock) {
    ...
    static int getRandomApplicationUid() {
        ...
        return ThreadLocalRandom.current().nextInt(android.os.Process.FIRST_APPLICATION_UID, android.os.Process.LAST_APPLICATION_UID + 1);
    }
}
