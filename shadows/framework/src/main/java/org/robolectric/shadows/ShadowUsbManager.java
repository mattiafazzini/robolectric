...
public class ShadowUsbManager {
    ...
    protected boolean hasPermission(UsbDevice device) {
        System.out.println("ShadowUsbManager#hasPermission");
    ...
    protected void grantPermission(UsbDevice device) {
        System.out.println("ShadowUsbManager#grantPermission");
    ...
    protected HashMap<String, UsbDevice> getDeviceList() {
        System.out.println("ShadowUsbManager#getDeviceList");
        return new HashMap<>(usbDevices);
    }

    @Implementation
    protected UsbAccessory[] getAccessoryList() {
        System.out.println("ShadowUsbManager#getAccessoryList");
    ...
    protected /* UsbPort[] */
    Object getPorts() {
        System.out.println("ShadowUsbManager#getPorts");
    ...
    public void addPort(String portId) {
        if (RuntimeEnvironment.getApiLevel() >= Build.VERSION_CODES.Q) {
            usbPorts.put((UsbPort) createUsbPort(realUsbManager, portId, UsbPortStatus.MODE_DUAL), (UsbPortStatus) createUsbPortStatus(UsbPortStatus.MODE_DUAL, UsbPortStatus.POWER_ROLE_SINK, UsbPortStatus.DATA_ROLE_DEVICE, 0));
            return;
        }
        usbPorts.put(callConstructor(UsbPort.class, from(String.class, portId), from(int.class, getStaticField(UsbPort.class, "MODE_DUAL"))), (UsbPortStatus) createUsbPortStatus(getStaticField(UsbPort.class, "MODE_DUAL"), getStaticField(UsbPort.class, "POWER_ROLE_SINK"), getStaticField(UsbPort.class, "DATA_ROLE_DEVICE"), 0));
    }

    @Implementation(minSdk = M)
    @HiddenApi
    protected /* UsbPortStatus */
    Object getPortStatus(/* UsbPort */
    Object port) {
        System.out.println("ShadowUsbManager#getPortStatus");
    ...
    Object dataRole) {
        System.out.println("ShadowUsbManager#setPortRoles");
        UsbPortStatus status = usbPorts.get(port);
        usbPorts.put((UsbPort) port, (UsbPortStatus) createUsbPortStatus(status.getCurrentMode(), (int) powerRole, (int) dataRole, status.getSupportedRoleCombinations()));
    ...
    protected ParcelFileDescriptor openAccessory(UsbAccessory accessory) {
        System.out.println("ShadowUsbManager#openAccessory");
        try {
            File tmpUsbDir = RuntimeEnvironment.getTempDirectory().createIfNotExists("usb-accessory").toFile();
            return ParcelFileDescriptor.open(new File(tmpUsbDir, "usb-accessory-file"), ParcelFileDescriptor.MODE_READ_WRITE);
    ...
    private static Object createUsbPortStatus(int currentMode, int currentPowerRole, int currentDataRole, int supportedRoleCombinations) {
        ...
        return callConstructor(UsbPortStatus.class, from(int.class, currentMode), from(int.class, currentPowerRole), from(int.class, currentDataRole), from(int.class, supportedRoleCombinations));
    }
    ...
    private static Object createUsbPort(UsbManager usbManager, String id, int supportedModes) {
        ...
        return callConstructor(UsbPort.class, from(UsbManager.class, usbManager), from(String.class, id), from(int.class, supportedModes));
    }
