...
public class ShadowMediaBrowserCompat {
    ...
    @Implementation
    protected void __constructor__(Context context, ComponentName serviceComponent, ConnectionCallback callback, Bundle rootHints) {
        System.out.println("ShadowMediaBrowserCompat#__constructor__");
        connectionCallback = callback;
        Shadow.invokeConstructor(MediaBrowserCompat.class, mediaBrowser, ClassParameter.from(Context.class, context), ClassParameter.from(ComponentName.class, serviceComponent), ClassParameter.from(ConnectionCallback.class, callback), ClassParameter.from(Bundle.class, rootHints));
    }

    @Implementation
    protected void connect() {
        System.out.println("ShadowMediaBrowserCompat#connect");
        handler.post(() -> {
            isConnected = true;
    ...
    protected void disconnect() {
        System.out.println("ShadowMediaBrowserCompat#disconnect");
        handler.post(() -> {
            isConnected = false;
        });
    }
    ...
    protected boolean isConnected() {
        System.out.println("ShadowMediaBrowserCompat#isConnected");
        return isConnected;
    }

    @Implementation
    protected String getRoot() {
        System.out.println("ShadowMediaBrowserCompat#getRoot");
        if (!isConnected) {
    ...
    protected void getItem(@NonNull final String mediaId, @NonNull final ItemCallback cb) {
        System.out.println("ShadowMediaBrowserCompat#getItem");
    ...
    protected void subscribe(@NonNull String parentId, @NonNull SubscriptionCallback callback) {
        System.out.println("ShadowMediaBrowserCompat#subscribe");
        subscribe(parentId, null, callback);
    }

    @Implementation
    protected void subscribe(@NonNull String parentId, @Nullable Bundle options, @NonNull SubscriptionCallback callback) {
        System.out.println("ShadowMediaBrowserCompat#subscribe");
        if (isConnected) {
            final MediaItem parentItem = mediaItems.get(parentId);
            List<MediaItem> children = mediaItemChildren.get(parentItem) == null ? Collections.emptyList() : mediaItemChildren.get(parentItem);
            handler.post(() -> callback.onChildrenLoaded(parentId, applyOptionsToResults(children, options)));
        } else {
     ...
     */
    @Implementation
    protected void search(@NonNull final String query, final Bundle extras, @NonNull SearchCallback callback) {
        System.out.println("ShadowMediaBrowserCompat#search");
        if (isConnected) {
    ...
    public MediaItem createMediaItem(String parentId, String mediaId, String title, int flag) {
        final MediaMetadataCompat metadataCompat = new MediaMetadataCompat.Builder().putString(MediaMetadataCompat.METADATA_KEY_MEDIA_ID, mediaId).putString(MediaMetadataCompat.METADATA_KEY_TITLE, title).putString(MediaMetadataCompat.METADATA_KEY_MEDIA_URI, Uri.parse(mediaId).toString()).build();
